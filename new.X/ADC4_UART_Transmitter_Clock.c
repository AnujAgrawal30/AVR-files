...
int main(void) {
    ...
    ADCSRA |= 1 << ADATE;                           // Auto Trigger Enable to run it according to clock
    sei();                                          // Setting up the global interrupts
    ADMUX = 0;                                      // Starting with first ADC
    ADCSRB |= 1 << ADTS2 | 1 << ADTS0;              // To select 16-bit Compare Match B
    ...
    TCCR1B |= 1 << WGM12;                           // To enable CTC mode
    TIMSK1 |= 1 << OCIE1B;                           // To enable Output Compare B match
    OCR1B = 15624;
    
    while (1) {
        if(ADCSRA & (1 << ADIF)){                   // To check if ADC conversion is complete
